...
public interface TableAudioNameDao extends BaseMapper<TableAudioName> {
    @ApiOperation("查询你的喜欢")
    @Select("select AUDIO_NAME_ID,audio_name_name,AUDIO_NAME_IMG,\n" +
            ...
            "u.U_ID=#{uid} and b.AUDIO_TYPE_ID=u.AUDIO_TYPE_ID order by R_NUM desc)\n" +
    ...
    @Select("select AUDIO_NAME_ID,audio_name_name,AUDIO_NAME_IMG,\n" +
            ...
            "table_audio_name b ORDER BY AUDIO_NAME_COUNT desc")
    IPage<TableAudioNameDTO> findAllTopBook(Page<TableAudioNameDTO> page);

    @ApiOperation("按大类型查看每一个大类型的排行")
    @Select("select AUDIO_NAME_ID,audio_name_name,AUDIO_NAME_IMG,\n" +
            "(SELECT U_NAME FROM table_user WHERE U_ID=b.u_id) uname from table_audio_name b\n" +
            "where ALL_TYPE_ID=#{allTypeId} ORDER BY AUDIO_NAME_COUNT desc LIMIT 0,5")
    List<TableAudioNameDTO> searchTopBookByType(int allTypeId);

    @ApiOperation("按大类型查看最新的5本书的信息")
    @Select("select AUDIO_NAME_ID,audio_name_name,AUDIO_NAME_IMG,(select U_NAME from table_user\n" +
            "where U_ID=b.U_ID) uname from table_audio_name b\n" +
            "where ALL_TYPE_ID=#{allTypeId} ORDER BY AUDIO_NAME_ID deSC  LIMIT 5;")
